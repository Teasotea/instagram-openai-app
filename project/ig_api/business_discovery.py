...
def getAccountPosts(params):
    """ Get posts of users account
	
	API Endpoint:
		https://graph.facebook.com/{graph-api-version}/{ig-user-id}?fields=business_discovery.username({ig-username}){media{caption}}&access_token={access-token}
	...
	"""
    endpointParams = dict() # parameter to send to the endpoint
    endpointParams['fields'] = 'business_discovery.username(' + params['ig_username'] + '){media{caption,timestamp}}' # string of fields to get back with the request for the account
    endpointParams['access_token'] = params['access_token'] # access token
...
if __name__ == "__main__":
    ...
    params = getCreds() # get creds
    params['ig_username'] = 'wintertwined'
    params['debug'] = 'no' # set debug
    response = getAccountPosts( params ) # hit the api for some data!

    print(len(response['json_data']['business_discovery']['media']['data']))
    
    for post in response['json_data']['business_discovery']['media']['data'][:2] :
        print("\n\n---------- POST ----------\n") # post heading
        print("\nPost caption:") # label
        print(post['caption']) # post caption
        print("\nTimestamp:") # label
        print(post['timestamp']) # post caption
        print("\nID:") # label
        print(post['id']) # type of media
